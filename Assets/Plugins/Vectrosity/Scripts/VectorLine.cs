        ...
        public void Draw3D()
        {
            ...
            for (int i = start; i < end; i += add)
            {
                ...
                if (USE_ZHENYI)
                {
                    ...
                    m_lineVertices[idx] = p1;
                    m_lineVertices[idx + 3] = p1;
                    m_lineVertices[idx + 2] = p2;
                    m_lineVertices[idx + 1] = p2;
                }
                else
                {
                    px.x = pos2.y - pos1.y; px.y = pos1.x - pos2.x;
            ...
            // zhenyi: not weld for now, it needs screen point position
            if (m_joins == Joins.Weld && end - start > 1)
            { // Only weld if there's more than one segment
                if (m_lineType == LineType.Continuous)
                {
